...
interface ComponentPayload {
  // 각 속성들은 쓰일수도 있고 안쓰일수도 있기때문에 선택적 체이닝(?) 사용
  tagName?: string
  props?: {
    [key: string]: unknown
  }
...
interface Route {
  path: string
  // 각 컴포넌트가 CoreComponent를 상속해서 씀 -> 왜 typeof?
...
function routeRender(routes: Routes) {
  ...
  // 2) 현재 라우트 정보를 찾아서 렌더링! - 실제 주소의 해쉬와 일치하는 부분을  찾기
...
// 제네릭을 사용해서 여러가지 타입을 받을 수 있도록
export class Store<S> {
  public state = {} as S // 상태(데이터)
  private observers = {} as StoreObservers // 클래스 내부에서만 쓰일거니 private
  constructor(state: S) {
